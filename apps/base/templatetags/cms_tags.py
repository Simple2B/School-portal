...
def get_navbars(context) -> "QuerySet[Navbar]":
    ...
    navbars = Navbar.objects.filter(navbarorderable__in=navbarorderables).order_by(
...
def get_footers(context) -> "QuerySet[Footer]":
    ...
    footers = Footer.objects.filter(footerorderable__in=footerorderables).order_by(
